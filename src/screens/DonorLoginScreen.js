...
export default function DonorLoginScreen({ navigation }) {
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      if (result.success) {
        showToast(`Welcome back, ${result.user.name || 'Donor'}! 🩸`);
        navigation.replace('Main');
      } else {
        showAlert('Login Failed', result.error || 'Invalid phone or password.');
      }
    } catch (e) {
      showAlert('Error', 'Something went wrong. Please check your internet connection.');
    } finally {
      setLoading(false);
    }
  };
      ...
      <View style={styles.container}>
        ...
        <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ padding: 24, paddingBottom: 60 }}>
          ...
          <View style={styles.passRow}>
            ...
            />
            <TouchableOpacity style={styles.eyeBtn} onPress={() => setShowPass(!showPass)} activeOpacity={0.7}>
...
const styles = StyleSheet.create({
  ...
  submitBtn: { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 8, backgroundColor: COLORS.red, borderRadius: RADIUS.full, paddingVertical: 16, marginTop: 28, elevation: 6 },
