...
export default function NotificationsScreen({ navigation }) {
  ...
  const formatTime = (createdAt) => {
    ...
    // Handle both numeric timestamp and legacy time string
    const date = typeof createdAt === 'number' ? new Date(createdAt) : new Date();
    const now = new Date();
    const diff = now - date;
  ...
  const renderItem = ({ item }) => (
    <View style={[styles.item, item.unread && styles.itemUnread]}>
      <View style={[styles.iconWrap, { backgroundColor: colorMap[item.type]?.bg || COLORS.blueLight }]}>
        <MaterialIcons name={iconMap[item.type] || 'info'} size={20} color={colorMap[item.type]?.color || COLORS.blue} />
      </View>
      <View style={styles.content}>
        <Text style={styles.title}>{item.title}</Text>
        <Text style={styles.desc}>{item.desc}</Text>
        <Text style={styles.time}>{formatTime(item.createdAt || item.time)}</Text>
      </View>
    </View>
  );
  ...
  return (
    <View style={styles.container}>
      <View style={styles.header}>
        ...
        <TouchableOpacity style={styles.markBtn} onPress={() => markAllRead?.()} activeOpacity={0.7}>
      ...
      <FlatList
        ...
        ListEmptyComponent={
          <View style={styles.empty}>
            ...
            <Text style={styles.emptySub}>You'll be notified when there are blood requests or updates</Text>
          </View>
        }
      />
    </View>
